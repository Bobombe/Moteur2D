...
void Moteur2D::run()
{
    ...
    {
        // On efface la fenètre dès le début pour permettre aux Updatables de dessiner
        ...
        {
            m_animatedTextures.at(i)->updateTexture(s);
        }
...
void Moteur2D::addAnimatedTextures(FixedSprite* fs)
{
    m_animatedTextures.push_back(fs);
}
...
void Moteur2D::deleteAnimatedTexture(FixedSprite*fs)
{
    for (unsigned int i = 0; i < m_animatedTextures.size(); i++)
    {
        if (fs==m_animatedTextures.at(i))
        {
            m_animatedTextures.erase(m_animatedTextures.begin()+i);
        }
    }
}
...
sf::Texture* Moteur2D::getTexture(const std::string& imagePath, bool reapetable)
{
    try
    {
